...
rules is HESO/1.0-conformant — it need not share code with the reference
implementation. The spec is reverse-extracted from the reference
implementation and pinned to the canonical test vectors in §1.9; those
...
### §0.1 What HESO proves — and what it does not
...
> anchor outside the operator and is **out of scope for HESO/1.0** — it is the
> domain of HESO Enterprise (§5).

This boundary is the whole reason for the trust grades in §5. HESO/1.0 is
**Grade 0**.
...
**every field present in the plat body is covered by the content hash (§1.8),
...
absent all hash differently (§1.9 V6a/V6b/V6c).
...
`steps`. Each step object has the shape (§1.4.1):
...
### §1.9 Canonical test vectors (conformance)
...
`plat_hash` (§1.8) like any other field. The determinism contract (§4) is:
**the same cassette MUST reproduce the same `plat_hash`.**

---

## §3 Sealed envelope (receipt)
...
signing convention (§3.5). They are not interchangeable.
...
2. `content.plat_hash` == recomputed BLAKE3 of `content` (§1.8), else
...
new `alg` tags — and are specified by HESO Enterprise, not this document:
...
1. canonicalizes per §1.7 (RFC 8785, no Unicode normalization);
2. computes `plat_hash` per §1.8 and reproduces the §1.9 vectors;
...
The §1.9 vectors are the minimum conformance suite.
