...
> vocabulary, hashed and versioned, that maps every action onto exactly one of
> **five destructive primitives**. This module is the crown jewel of the open
> standard: the thing third parties classify against, the schema the receipt
> envelope binds to, and the only thing in HESO/1 that decides "what did this
> action *do*."

The gold-master data this prose defines lives next to it as
[`../taxonomy.toml`](../taxonomy.toml) (lifted verbatim from the kernel; never
hand-edited — see the [vendor contract](#8-conformance-and-the-vendor-contract)).
The prose is the normative interpretation; the TOML is the data; the kernel is
the reference implementation. A clean-room re-implementer MUST be able to build a
byte-identical classifier from this module plus `taxonomy.toml` plus the
published `taxonomy-classify` vectors, with no access to the kernel source.
...
Sibling modules: [action-receipt.md](./action-receipt.md) (carries the
classification + the pinned hash), [envelope.md](./envelope.md) (the in-toto
predicate whose schema *is* this taxonomy), [transparency.md](./transparency.md)
(the log the classified receipt is admitted to).
...
This module's canonical spine reconciles with the kernel's shipped **frozen-7
coarse verbs** in §6. The verbs are a descriptive sub-vocabulary that maps onto
the five; the **five primitives, not the verbs, are canonical**. This resolution
is locked by [ADR 0001 — taxonomy spine](https://github.com/hesohq/heso-redesign/blob/redesign/decisions/0001-taxonomy-spine.md).
Read §6 before assuming the verbs and primitives are 1:1 — they are close but not
identical.
...
| `row_threshold` | the observed row/record count | `threshold` (matches when `row_count_estimate >= threshold`; UNKNOWN fails SAFE into the stricter lane) | `rows >= 1000` |
...
- **`row_threshold` and `fact_flag` are what make the taxonomy structural rather
  ...
  regardless of which endpoint served it; "this host is a secret store" is a fact
  resolved once upstream and reused. UNKNOWN inputs MUST fail SAFE — i.e. resolve
  toward the *stricter* lane, never the laxer one.
...
- **Across rows, classes OR** — a class matches if **any** of its rows matches.
  This is how "host ∈ {stripe, …} OR payment-API path grammar OR the `is_payment`
  body fact ⇒ `payment_endpoint`" is encoded as three rows on one class.
...
parameter sets (host lists, path globs, tokens) are defined **once** in the TOML
and MUST NOT be restated here — this table is the index, the TOML is the data.

| # | Class `id` | `coarse_verb` | `effect` | Canonical primitive (§6) | Matched by |
|---|---|---|---|---|---|
| 1 | `payment_endpoint` | `payment` | `spend` | **`move-value`** | payment host set · payment-API path grammar · `is_payment` fact |
| 2 | `destructive_op` | `delete` | `destroy` | **`destroy`** | `DELETE` method · destructive argv/SQL tokens · `effect_destructive` fact |
| 3 | `identity_endpoint` | `account_change` | `grant` | **`change-authority`** | IAM/auth/SSO host set · permission-mutation path grammar · `is_identity_change` fact |
| 4 | `secret_store` | `data_export` | `transfer_out` | **`disclose`** | secret-manager host set · secret-path globs · `is_secret` fact |
| 5 | `bulk_data` | `data_export` | `transfer_out` | **`disclose`** | `row_count_estimate >= 1000` (UNKNOWN fails SAFE) |
| 6 | `model_endpoint` | `llm_call` | `observe` | **`observe`** (default) | inference-provider host set · inference path grammar · `is_model_call` fact |
| 7 | `messaging_endpoint` | `http_request` | `transfer_out` | **`execute`** / **`observe`** | comms host set · comms path grammar |
...
| 10 | `unresolved` | `tool_call` | `effect_unknown` | **`residual`** | `always` (unconditional residual) |
...
classification of record and is derived per §6.
...
```
taxonomy_hash = BLAKE3( RFC8785-JCS( normative_projection(parsed_taxonomy) ) )
```

Computed in three steps, in this order:

1. **Parse + validate** `taxonomy.toml` into the in-memory taxonomy (totality and
   closed-vocabulary checks of §4 MUST pass first; an invalid taxonomy has no
   hash).
2. **Normative projection.** Project the parsed taxonomy to a JSON value that
   contains **only the normative classification data** and **nothing else**:
   - **Include:** the ordered list of classes, each with its `id`,
     ...
     is semantic, §2.3) and MUST be preserved.
...
or ordering changes. The `taxonomy-classify` conformance vectors
([conformance — §2.2](https://github.com/hesohq/heso-redesign/blob/redesign/standard/conformance-and-envelope.md))
include the expected `taxonomy_hash`, so an implementation that canonicalizes or
hashes differently is caught mechanically.

### 5.2 Versioning policy — pin at signing

Versioning is locked by
[ADR 0012 — pin at signing](https://github.com/hesohq/heso-redesign/blob/redesign/decisions/0012-taxonomy-versioning-pin-at-signing.md).
...
The mapping below is reproduced **verbatim and normatively** from
[ADR 0001](https://github.com/hesohq/heso-redesign/blob/redesign/decisions/0001-taxonomy-spine.md),
which is the single source of truth and is immutable until superseded by a
higher-numbered ADR:
...
**add-only**, **namespaced**, and **monotonic**.
...
- A customer extension `id` MUST be namespaced `<ns>/<name>` (e.g.
...
- An extension **MAY add members** to a built-in class's predicate sets via
  `[[extend]]` (more hosts, suffixes, globs, or tokens) — this strictly **grows**
  what is caught, which is always safe.
...
2. **Class registration.** Registering an extension class means contributing (a)
   its namespaced `id` + the primitive it maps to, (b) its predicate rows over
   the **same closed vocabulary** (§2 — no new predicate kinds), and (c)
   `taxonomy-classify` vectors proving its behavior. Registration is a spec-repo
   change with vectors, **not** a private edit to a vendored copy.
...
- **One canonical home.** This module + [`../taxonomy.toml`](../taxonomy.toml) are
  the only normative home of the taxonomy. The kernel classifier, every SDK, the
  gate, and the conformance vectors point here; nothing restates the spine.
- **Vendored via pinned-sha + drift gate.** Implementation repos MUST vendor
  `taxonomy.toml` pinned to a specific `heso-spec` commit sha, and a CI drift gate
  MUST fail the build if any vendored copy differs from `heso-spec@<sha>`. Impl
  repos MUST NOT hand-copy or fork the TOML.
- **Proven by vectors.** The `taxonomy-classify` goldens (structural input →
  ...
  surface MUST all agree on them. See
  [conformance-and-envelope.md](https://github.com/hesohq/heso-redesign/blob/redesign/standard/conformance-and-envelope.md).
- **Bound into the envelope.** The classification is the **predicate** of the
  in-toto Statement carried by the DSSE envelope (`predicateType`
  `https://hesohq.dev/ActionReceipt/v2`) — **the predicate schema *is* this
  taxonomy**. See [envelope.md](./envelope.md).

---

## 9. Pointers

- **Gold-master data** — [`../taxonomy.toml`](../taxonomy.toml) (lifted verbatim;
  never hand-edited).
- **Sibling modules** — [action-receipt.md](./action-receipt.md) ·
  [envelope.md](./envelope.md) · [transparency.md](./transparency.md)
- **The locked decisions** —
  [ADR 0001 (spine)](https://github.com/hesohq/heso-redesign/blob/redesign/decisions/0001-taxonomy-spine.md) ·
  [ADR 0009 (in-toto/DSSE)](https://github.com/hesohq/heso-redesign/blob/redesign/decisions/0009-in-toto-dsse-envelope.md) ·
  [ADR 0012 (pin at signing)](https://github.com/hesohq/heso-redesign/blob/redesign/decisions/0012-taxonomy-versioning-pin-at-signing.md)
- **The why** — classify-by-effect, determinism + totality, the 5-vs-7 argument:
  [taxonomy-rationale.md](https://github.com/hesohq/heso-redesign/blob/redesign/standard/taxonomy-rationale.md)
