...
This module is the *format and proof* spec. It defines the leaf shape, the tree hashing, the proof verification rules, and the interop reference vectors. It is deliberately silent on *who operates the log* and *how proofs are transported*; the historical witnessed-network topology (checkpoints, cosignatures, tile serving) is summarised for history, not specified, in [§6](#6-the-witnessed-network-forward-slot-superseded).
...
This module specifies the **format and the offline proof math** ([§2](#2-leaves)–[§5](#5-interop-reference-vectors)) — the part a relying party needs to verify proofs it is handed. The superseded **witnessed network** forward slot would have produced and served those proofs through checkpoint signing, witness cosignatures, the C2SP signed-note / `tlog-checkpoint` wire format, and tile serving.
...
- [`vectors/`](../vectors/) and [`verifier/`](../verifier/) — the `rfc6962-roots` vectors and clean-room verifier surface.
- [§6](#6-the-witnessed-network-forward-slot-superseded) — the historical witnessed-network forward slot. The completeness story it backed is dropped under the positive-receipt model; the offline proof math here is unaffected.
- [Governance §4](../GOVERNANCE.md#4-taxonomy_hash-and-compatibility-gating) — the immutable-version discipline mirrored in [§7](#7-versioning).
